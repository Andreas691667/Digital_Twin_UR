...
class UR5e_RL(UR5e):
    ...
    def compute_fk(self, j1, j2, j3, j4, j5, j6):
        ...
        T_4_5 = np.array(
            [
                [np.cos(j5), -np.sin(j5), 0, 0],
                [0, 0, -1, -d5],
                [np.sin(j5), np.cos(j5), 0, 0],
                [0, 0, 0, 1],
            ]
        )
        T_5_6 = np.array(
            [
                [np.cos(j6), -np.sin(j6), 0, 0],
                [0, 0, 1, d6],
                [-np.sin(j6), -np.cos(j6), 0, 0],
                [0, 0, 0, 1],
            ]
        )
        ...
        T = SE3(
            trnorm(
                np.array(
                    [
                        [1, 0, 0, x],
                        [0, np.cos(5 * np.pi / 4), -np.sin(5 * np.pi / 4), y],
                        [0, np.sin(5 * np.pi / 4), np.cos(5 * np.pi / 4), z],
                        [0, 0, 0, 1],
                    ]
                )
            )
        )  # Rotation of 5*pi/4 on the x-axis
    ...
    def compute_ik_analytic(self, x, y, z):
        ...
        T = np.array(
            [[0, 0, 1, x], [0.5, -0.866, 0, y], [0.866, 0.5, 0, z], [0, 0, 0, 1]]
        )
        # T = SE3(trnorm(T))
        x = T[0][3]
        y = T[1][3]
